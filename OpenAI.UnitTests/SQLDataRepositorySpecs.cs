...
namespace OpenAI.UnitTests
{
    ...
    public class SQLDataRepositorySpecs
    {
        ...
        {
            var articlesForProcessing = sqlDataRepository.GetArticlesForProcessing();
            List<EmbeddedArticlesDTO> embeddedArticlesDTOs = new List<EmbeddedArticlesDTO>();
            foreach (var articles in articlesForProcessing)
            {
                var htmlInput = codeUtility.RemoveScriptsStylesAndHtml(articles.FormattedBody);
            ...
            foreach (var embeddedArticles in embeddedArticlesDTOs)
            {
                ...
                foreach (var data in embedResponse.Data)
                {
                    ...
                    PineConeVectorsDTO pineConeVectors = new PineConeVectorsDTO();
                    pineConeVectors.ID = GenerateVectorID(embeddedArticles, EmbededIndex);
                    pineConeVectors.Values = data.Embedding;
                    pineConeVectors.Metadata = new PineConeMetaDataDTO() { Url = embeddedArticles.url, Text = embeddedArticles.texts[EmbededIndex], SplitType = embeddedArticles.splitType };
                    PineConerequestDto.Vectors.Add(pineConeVectors);
                }
            }
